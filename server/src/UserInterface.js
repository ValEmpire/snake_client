...
class UserInterface {
  ...
  updateScore(snakes) {
    ...
    if(snakes.length > 0){
      // map the score for each snakes
      currentScoreBoard = snakes.map(snake=>{

        // check if highestScore is greater than the current score
        if(snake.score > this.highestSnake.score){
          this.highestSnake = snake;
        }
        return `${snake.name}:${snake.score}`
      });
      // this.scoreContainer.setLine(0, `{bold}Score:{/bold} ${score}`)
    }

    this.scoreContainer.setLine(0, `{bold}IP:{/bold} ${ip.address()} {bold}PORT:{/bold} ${PORT} ${currentScoreBoard?`{bold}CURRENT SCORE:{/bold} ${currentScoreBoard}`:''} {bold}HIGHEST SCORE:{/bold} ${this.highestSnake.name !== null ? `${this.highestSnake.name}:${this.highestSnake.score}`:0} {bold}Connected Players:${snakes.length} {/bold}`)

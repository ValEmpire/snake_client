...
class Game {
  ...
  handlePlayerInput(input, client) {
    ...
    if (dir) {
      ...
      // I moved the checkPlayerHits method here
      // this will check if player hits something once they moved
      ...
      return snake.changeName(name[1].trim().substring(0, MAX_PLAYER_NAME_LENGTH), () => {
        // this will set the scoreboard name everytime newPlayer connects
        this.ui.updateScore(this.snakes)

        // this will send the message to all other players that this player name joined the game
        this.snakes.forEach(s => client === s.client ? '' : s.client.write(`${snake.name} joined the game.`))
      ...
      if (SNAKE_COLLISIONS) {
        for (let [j, otherSnake] of this.snakes.entries()) {
          ...
          if (i !== j && snake.hitSnake(otherSnake)) {
            // head on collision
            // I think it is fair to kill all snakes if theres a head on collision
            if(snake.segments[0].x === otherSnake.segments[0].x && (snake.segments[0].y) === otherSnake.segments[0].y){
              this.removeSnake(snake, i, 'head on collision, so you ded.')
              this.removeSnake(otherSnake, j, 'head on collision, so you ded.')
              return;
            }
  ...
  tick() {
    ...
    this.generateDots()
    this.drawDots()
    if (this.autoMove) this.moveSnakes(this.snake)
    this.drawSnakes()
    this.ui.render()
  }
